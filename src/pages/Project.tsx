  ...
  return (
    <Card
      className={`scroll-reveal transition-all duration-300 border-2 ${bin.borderColor} cursor-pointer bg-card ${
        isActive 
          ? 'shadow-xl scale-105 -translate-y-2 border-primary/40' 
    ...
    >
      <CardContent className="pt-6 pb-6 md:pt-8 md:pb-8 text-center relative z-10">
        ...
        `}>
          <Icon className={`w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 ${bin.color}`} />
        </div>
...
export default function Project() {
  ...
  const handleBinMouseEnter = useCallback((index: number) => {
    setActiveBinIndex(index);
  }, []);

  // Bin rotation effect with cleanup
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-primary/5 overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-12 md:py-16 relative z-10">
        ...
        <div className="max-w-5xl mx-auto text-center mb-16 md:mb-24">
          ...
          >
            <Link to="/guide" aria-label={language === "fr" ? "Découvrir le projet" : "Discover the project"}>
              <Button 
                size="lg" 
            ...
            <Link to="/resources" aria-label={language === "fr" ? "Voir les ressources" : "View resources"}>
              <Button 
                variant="outline" 
                size="lg" 
          ...
          <div className="mt-12 animate-bounce">
            <ChevronDown className="w-6 h-6 mx-auto text-muted-foreground" aria-hidden="true" />
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {bins.map((bin, index) => (
              <BinCard
                ...
                onMouseEnter={() => handleBinMouseEnter(index)}
              />
            ))}
          </div>
        </div>
          ...
          <Card className="border-2 border-primary/30 shadow-xl overflow-hidden group hover:shadow-2xl transition-shadow duration-300">
            ...
            <CardContent className="p-10 md:p-12 relative">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/guide" aria-label={t("project.startLearning") || (language === "fr" ? "Commencer à apprendre" : "Start learning")}>
                ...
                <Link to="/activities" aria-label={language === "fr" ? "Jouer aux jeux" : "Play games"}>
